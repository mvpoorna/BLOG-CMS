...
export class HomeService {
  ...
  likePost(id: string, likes: number): Observable<Post> {
    likes = likes+1;
    const postData = {id: id,likes: likes};
    return this.http.post<Post>(apiUrl + 'post/like',postData).pipe(
      tap(_ => console.log(`update Like count for post by id=${id}`)),
      catchError(this.handleError<Post>(`likePost id=${id}`))
    );
  }
